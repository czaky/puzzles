...
def parse_binop_expression(tokens: Sequence) -> tuple:
...
def format_binop_expression(e: str | tuple) -> str:
    ...
    def fop(op: str, a: str | tuple, b: str | tuple) -> tuple:
        ...
        def fl(v):
            if isinstance(v, str) or op in "+-" or v[0] in "*/":
                return f(v)
            return "(" + f(v) + ")"
...
def simplify_binops_brackets_one_pass(tokens: Sequence) -> str:
    """Simplifies an arithmetic expression consisting of literals and operators from '+-*/()'.

    This parses a binary opertation expression and formats it in one-pass.

    Parameters:
        tokens (list): tokenized expression consisting of operators '+-*/()' and literals.

    Returns:
        str: a string with redundant brackets removed
    """
    # The implementation here avoids recursion and uses two stacks
    # similar to the  Dijkstra's Shunting Yard algorithm like above.
    # The difference to `parse_binop_expression` is that the format operation
    # is immediately applied without generating the intermediate syntax expression tuples.
    ...
    def close_bracket():
        ...
        # Apply the operators (in reversed order) to form the expressions.
        while rops:
            rargs.append(fop(rops.pop(), rargs.pop(), rargs.pop()))
        # The final expression is moved back onto the stack.
        args.append(rargs.pop())
